...
public class BinaryNode<T> {
    ...
    public String toString() {
    	String str = "";
    	if (left != null) {
    		str = str + left.rowString(1); //adds the root's left leaf data to the string
    	}
    	str = str + data.toString() + "\n";
    	if (right != null) {
    		str = str + right.rowString(1); //adds the root's right leaf data to the string
    	}
    	
    	return str;
    }
    
    public String rowString(int depth) {
    	String str = "";
    	if (left != null) {
    		str = str + left.rowString(depth + 1); //adds the left root's leaf data to the string as row
    	}
    	str = str + spaces(depth) + data.toString() + "\n";
    	if (right != null) {
    		str = str + right.rowString(depth + 1); //adds the right root's leaf data to the string as row
    	}
    	return str;
    }
    
    //The following method returns the number of spaces for each depth
